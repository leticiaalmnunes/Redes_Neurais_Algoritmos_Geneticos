...
def gene_palavra(letras_possiveis):
    """Sorteia uma letra.
...
def cria_palavra(letras_possiveis, tamanho_pal):
    palavra = []
    vogais = ["a","e","i","o","u"]
    
    for _ in range(tamanho_pal):
        palavra.append(gene_palavra(letras_possiveis))
        
    if any(i in palavra for i in vogais):
        pass     
    else:
        posicao = random.randint(0,tamanho_pal-1)
        palavra[posicao] = random.choice(vogais)
        
    return palavra


def populacao_palavras(letras_possiveis, tamanho_pal, tamanho_populacao):
    populacao = []

    for _ in range(tamanho_populacao):
        populacao.append(cria_palavra(letras_possiveis, tamanho_pal))

    return populacao

                       
def funcao_objetivo_palindromo(string, tamanho, percent=0):
    """Checa o quanto a string de entrada é um palíndromo."""  
    ...
    if (len(string)==tamanho) and (not any(i in string for i in vogais)):
        return 1/tamanho
    else:
        return funcao_objetivo_palindromo(string[1:-1], tamanho, percent=percent) 


def funcao_objetivo_pop_palindromo(populacao, tamanho, percent=0):
    ...
    for individuo in populacao:
        fitness.append(funcao_objetivo_palindromo(individuo, tamanho, percent=0))
